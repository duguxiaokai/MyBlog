...
'sql'=>"select * from (SELECT a.id,b.name as executername,count(a.id) as week_worknumber,executerid
		...
		WHERE  date_format(begindate,'%Y-%m')=date_format(curdate(),'%Y-%m')
                AND date_format(enddate,'%Y-%m')=date_format(curdate(),'%Y-%m')
		...
		WHERE  date_format(begindate,'%Y-%m')=date_format(curdate(),'%Y-%m')
                AND date_format(enddate,'%Y-%m')=date_format(curdate(),'%Y-%m')
		AND a.cat = '周报' AND a.finishper =1 group by b.name
