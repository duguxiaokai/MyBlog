<?php return array(
'id'=>"oa_task_tongji",
'title'=>"日报列表",
'tablename'=>"oa_task",
'filter'=>"SYS_ORGID=orgid()",
'orderby'=>"executerid desc",
'groupby'=>"y",
'actions'=>"add,del,open,edit,search,page",
'fields'=>array(
 array('id'=>"id",'name'=>"ID",'fieldtype'=>"int",'isvisible'=>"false",'visibleWhenAdd'=>"false",),
 array('id'=>"projectid",'name'=>"项目",'fieldtype'=>"string",'type'=>"dropdown",'data'=>"sql:select id,name from oa_project",'isvisible'=>"false",'visibleWhenAdd'=>"false",),
 array('id'=>"name",'name'=>"任务名称",'fieldtype'=>"string",'forsearch'=>"true",'nullable'=>"false",),
 array('id'=>"cat",'name'=>"类型",),
 array('id'=>"tasktype",'name'=>"工作类别",'fieldtype'=>"string",'type'=>"dropdown",'isvisible'=>"false",'visibleWhenAdd'=>"false",'forsearch'=>"true",),
 array('id'=>"begindate",'name'=>"开始日期",'fieldtype'=>"date",'type'=>"date",'forsearch'=>"true",),
 array('id'=>"enddate",'name'=>"结束日期",'fieldtype'=>"date",'type'=>"date",),
 array('id'=>"detail",'name'=>"任务描述",'fieldtype'=>"string",'type'=>"textarea",'isvisible'=>"false",),
 array('id'=>"executerid",'name'=>"执行人",'fieldtype'=>"string",'type'=>"dropdown",'data'=>"sql:select id,name from sys_staff",'defaultdata'=>"Me()",),
 array('id'=>"status",'name'=>"状态",'fieldtype'=>"string",'type'=>"dropdown",'data'=>"plan:计划,doing:执行,done:完成,better:优化,checked:验收,cancel:取消",),
 array('id'=>"finishper",'name'=>"完成状态",'type'=>"radio",'data'=>"0:进行中,1:已完成",'defaultdata'=>"0",),
 array('id'=>"price",'name'=>"单价",'isvisible'=>"false",'visibleWhenAdd'=>"false",),
 array('id'=>"totalprice",'name'=>"总价",'isvisible'=>"false",'visibleWhenAdd'=>"false",),
 array('id'=>"notes",'name'=>"备注",'fieldtype'=>"string",),
 array('id'=>"SYS_ORGID",'name'=>"组织ID",'fieldtype'=>"int",'type'=>"SYS_ORGID",'isvisible'=>"false",'visibleWhenAdd'=>"false",'readonly'=>"true",),
),

) ?>