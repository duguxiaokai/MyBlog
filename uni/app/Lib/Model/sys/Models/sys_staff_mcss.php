<?php return array(
'id'=>"sys_staff_mcss",
'title'=>"员工信息",
'tablename'=>"sys_staff",
'cols'=>"2",
'fields'=>array(
 array('id'=>"id",'name'=>"ID",'fieldtype'=>"int",'isvisible'=>"false",'visibleWhenAdd'=>"false",),
 array('id'=>"name",'name'=>"员工姓名",'fieldtype'=>"string",),
 array('id'=>"appellation",'name'=>"员工称谓",'fieldtype'=>"string",),
 array('id'=>"gender",'name'=>"性别",'fieldtype'=>"string",'type'=>"radio",'data'=>"M:男,F:女",'defaultdata'=>"M",),
 array('id'=>"birthday",'name'=>"出生日期",'fieldtype'=>"date",'type'=>"date",),
 array('id'=>"mobile",'name'=>"手机号码",'fieldtype'=>"string",),
 array('id'=>"tel",'name'=>"联系电话",'fieldtype'=>"string",),
 array('id'=>"msn",'name'=>"MSN",'fieldtype'=>"string",),
 array('id'=>"no",'name'=>"员工编号",'fieldtype'=>"string",),
 array('id'=>"qq",'name'=>"QQ",'fieldtype'=>"int",),
 array('id'=>"statue",'name'=>"在职状态",'fieldtype'=>"string",'type'=>"radio",'data'=>"1:在职,0:离职",'defaultdata'=>"1",'readonly'=>"true",),
 array('id'=>"email",'name'=>"Email",'fieldtype'=>"string",),
 array('id'=>"dept",'name'=>"所在部门",'isvisible'=>"false",'visibleWhenAdd'=>"false",'readonly'=>"true",'virtualfield'=>"true",),
 array('id'=>"deptid",'name'=>"部门",'fieldtype'=>"int",'type'=>"dropdown",'data'=>"sql:select id,name from sys_dept where SYS_ORGID=orgid()",'readonly'=>"true",),
 array('id'=>"post",'name'=>"岗位",'type'=>"popupselectone",'data'=>"model:sys_select_position",'readonly'=>"true",),
 array('id'=>"notes",'name'=>"备注",'fieldtype'=>"string",'type'=>"textarea",),
),

) ?>