...
*/
g_dinamic_cssjs.push("jusaas/js/JSData.js");
...
function MCSSData(data,modeldata,params)
{
	...
	//从最初数据中，获得某记录的某字段的值
	this.getFieldOldValue=function(recordid,fieldid)
	{
		for(var i=0;i<this.initData.length;i++)
		{
			if (this.initData[i][this.keyfield]==recordid)
				return this.initData[i][fieldid];
		}
		return null;
	}
	...
	this.editNewData=function(record)
	{
		...
		for(var i=0;i<this.newData.length;i++)
		{
			if (this.newData[i].recordid==record.recordid)
			{
				...
				//this.newData[i]['mcss_recordtype']=record.mcss_recordtype;//这行需要吗？
					...
					else
					{
						if (showhint)
						{
							if (data=='0')
								...
								mcdom.alert(lang.successfullysaved,lang.save,'info','fadeout',250,100,{seconds:1});
							}
						}
						_this.state="SAVED";
						_this.newData=[];
