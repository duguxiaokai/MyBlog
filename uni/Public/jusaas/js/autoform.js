...
g_dinamic_cssjs.push("jusaas/js/autoform.js");
...
function Autoform(formid,jason)
{
	...
	this.typename='Autoform';

	//设置默认参数
	jason.homeUrl=getHomeUrl();
	if (!jason.saveUrl)
		jason.saveUrl=jason.homeUrl+g_systempath+"/saveRecord";
	if (!jason.getRecordUrl)
		jason.getRecordUrl=jason.homeUrl+g_systempath+"/getOneRecord";
	if (!jason.getModelUrl)
		jason.getModelUrl=jason.homeUrl+g_systempath+"/getModelData";
	...
	this.richeditorid=null;//这个有局限，当有多个富文本字段时智能保存其中一个字段的值
	...
	//run方式是核心。开始创建表单。参数func指定的方法会在创建完表单元素后和加载数据后分别执行
	...
	{	
		var s="<img id='loading' src='"+g_rooturl+"/Public/Images/loading.gif'/>";
		$("#"+this.id).before(s);
...
//				mcss_importJS("jusaas/js/viewImage.js");
	...
	//解析模型，主要是根据参数中的字段默认值，修改模型的字段默认值
	...
	//根据模型字段定义，取出最常用的字段属性，写到类的arr_json中。备注：arr_json属性名称不恰当
	...
	this.createButtons=function()
	{
		...
		if (this.params.showFormButton.indexOf("SAVE")>-1)
			h+="<div class='smallloginsubmit'><input type='button' class='btn btn-green mcssingbutton' id='"+this.formid+"_savebutton' value='保存' ></div>";
	...
	//长度校验
	this.verifylength=function(obj)
	{ 
		if(obj)
		{
			...
			if(fieldModel)
			{
			    ...
			    else if(fieldModel.type=='richeditor')
				{  
					var v=this.getFieldValue(fieldModel.id);
					var v=v.replace(/<[^>]+>/g,"");
					if (v.length>fieldModel.length)
					{   						
					  var err=fieldModel.name+":"+lang.maxcharlength+fieldModel.length;
					  this.showErr(obj,err);
	...
	{    
		...
		if(!this.hidetitle)
		{
			...
			{
				s+="<td class='lefttitle'><span style='color:red'> * </span>"+name+"&nbsp;&nbsp;</td>";
			}
			else
			{
				s+="<td class='lefttitle'>"+name+"&nbsp;&nbsp;</td>";
			}
		}
	...
	this.get_richeditor=function(field)
	{
		...
		d="<div id='"+this.formid+"_"+field.id+"_div' style='width:"+width+";'>";
 		if (field.readonly == "true")
		{
	...
	//获得富文本框id，用于改变样式等
	this.getRichEditorDivId=function(fieldid)
	{
		return this.formid+"_"+fieldid+"_div";
	}
	...
	//创建普通输入框,包括普通文本框、日期、时间
	this.get_inputbox=function(field,width)
	{  
	    var d="";
	...
	//把模型字段需要用到的属性提交出来。因此如果模型新增了属性，需要修改这个方法。备注：这个做法并不好
	...
	this.getValue=function(item)
	{
		var newValue='';
		    ...
		    if (item.className.indexOf(this.formid+"_richeditor")>-1)
			{   
				//newValue=item.innerHTML;
				newValue=$(item).prev().children().find("iframe").contents().find("body").html();
				if (!newValue)
					newValue=$(item).val();
			}				
			else
		...
		for(y in FieldArr)
		{
 			...
 			for(var i=0;i<list.length;i++)
			{
				...
				{	
					...
					if (FieldArr[y].type=="checkboxlist")
					{
					    for(var m=0;m<checkboxs.length;m++)
					    {
							if (checkboxs[m].checked)
   ...
   //获取radio的值
	...
	this.save=function(refresh,func,silent,params)
	{
		...
		if(this.checkuniquebeforesave)
		{
			...
			if (unique.length>0)//检查table中是否有唯一的字段--刘兆菊
			{
				...
				var _this=this;
				$.post(this.homeUrl+g_systempath+"/checkUnique",{modelid:this.modelid,field:field,value:value,recordid:this.recordid},function(isunique)
				{
	...
	{	
		...
		if (!this.canClickSave)
		{
 		...
 		//this.fieldvalues=this.getValues();//这个已经在judgevaluechanged执行，多余了
		...
		{  
			...
			if (this.modeldata.editurl=="default") 
				this.modeldata.editurl = g_systempath+"/addRecord/model/"+this.modelid+"/tablename/"+this.tablename;
			...
			var _this=this;//多个类都用到_this变量，注意它的有效性
			...
			$.post(this.saveUrl,param, function(data)
			{
				...
				if (data=="" || arr.length==2)
				{
					if (!silent){
						...
						MCDom.prototype.alert(lang.successfullysaved,lang.save,"info","fadeout",'300','200');
					}
		...
		else 
		{
			...
			if (func)
				func(this.recordid,lang.noupdates,this);	
		};		
	}
    ...
    this.getFieldValue=function(fieldid)
	{
		...
		if (id)
		{
			...
			var v=$("#"+id).val();
			if (!v || f.type=="richeditor")//richeditor类型的字段，有个附加的textarea字段，因此有值，但不是最新的。在getValue方法中取值
...
{ 
    ...
    if(field.title!=null)
	{
		...
		var url=homeUrl+g_systempath+"/RichEditor?srcobj="+id+"&&readvalue="+paramvalue[1];
		window.open(url);
    }
...
function autoform_check_unique(formid,obj,modelid,field,value)
{
	...
	$.post(f.homeUrl+g_systempath+"/checkUnique",{modelid:modelid,field:field,value:value,recordid:id},function(isunique){
...
Autoform.prototype.deleteRecord=function(){
	$.post(this.homeUrl+g_systempath+"/deleteRecord",{recordid:this.recordid,modelid:this.modelid},function(result){
		//alert(result);
		if (!result)
			alert(result);
...
function mcform_showselectview(obj,width,FieldText,FieldValue,sql){
	...
	if(sql){
		...
		if(getCookie(cookiekey)){
			...
			$.post(getrooturl()+"/index.php/System/Model/getValueByKey",{cookiekey:cookiekey},function(data){
				if(data){
