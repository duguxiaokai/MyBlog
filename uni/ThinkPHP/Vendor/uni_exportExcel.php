<?php 
	...
	function exportExcel($data="",$title="",$display="",$path="",$filetype="xls"){
		...
		foreach($title as $value){
			...
			$objActSheet->getColumnDimension($array[$i-1])->setWidth(15);//设置每列日期宽度为15
			if(!$display[$i-1])
		...
		while ($dt_start<=$dt_end){//循环甘特图日期
			...
			if($dt_start == strtotime($begindate) || $dt_start == strtotime($enddate)){
				$objActSheet->getColumnDimension($array[$i])->setAutoSize(true); 
				$objActSheet->getColumnDimension($array[$i])->setWidth(15);//设置每列日期宽度为15
			...
			while ($dt_start<=$dt_end){
				...
				if($dt_start>=strtotime($data[$i]['begindate']) && $dt_start<=strtotime($data[$i]['enddate'])){
					...
					if($data[$i]['finishper']==0)//根据完成率填充不同颜色
						$objFill->getStartColor()->setARGB('333333');
					else if($data[$i]['finishper']==100)
						$objFill->getStartColor()->setARGB('0033cc');
					else
						$objFill->getStartColor()->setARGB('33ff00');
				}
				$dt_start = strtotime('+1 day',$dt_start);
				$gantedate++;
			}
