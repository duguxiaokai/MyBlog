<?php 
     ...
     */  
	function exportExcel($data="",$title="",$display="",$path="",$filetype="xls"){
		...
		// 创建一个处理对象实例
		$style=array(
		header_bgd_color=>'FFEEEEEE',//表头背景色
		header_border_line=>PHPExcel_Style_Border::BORDER_THIN,////表头上方线条样式
		...
		foreach($title as $value){
			...
			$objActSheet->getStyle($array[$i-1].'1')->getBorders()->getTop()->setBorderStyle($style['header_border_line']); 
			$objActSheet->getStyle($array[$i-1].'1')->getBorders()->getLeft()->setBorderStyle($style['header_border_line']); 
			$objActSheet->getStyle($array[$i-1].'1')->getBorders()->getRight()->setBorderStyle($style['header_border_line']); 
			$objActSheet->getStyle($array[$i-1].'1')->getBorders()->getBottom()->setBorderStyle($style['header_border_line']); 
			//Set border colors 设置边框颜色 
			$objActSheet->getStyle($array[$i-1].'1')->getBorders()->getLeft()->getColor()->setARGB('000000'); 
			$objActSheet->getStyle($array[$i-1].'1')->getBorders()->getTop()->getColor()->setARGB('000000'); 
			$objActSheet->getStyle($array[$i-1].'1')->getBorders()->getRight()->getColor()->setARGB('000000'); 
			$objActSheet->getStyle($array[$i-1].'1')->getBorders()->getBottom()->getColor()->setARGB('000000'); 
			//设置垂直居中对齐
		...
		$datecell = $i-1;//记录日期开始的键值
		$dateendcell;//记录日期结束的键值
		$begindate = '2013-08-1';//开始日期
		$enddate = '2013-08-31';//结束日期
		...
		while ($dt_start<=$dt_end){//循环甘特图日期
			if($dt_start==$dt_end)
				$dateendcell = $i;
			$w = date('w',$dt_start);
			$objActSheet->getStyle($array[$i-1].'1')->getAlignment()->setWrapText(true);
			$objActSheet->getStyle($array[$i-1].'1')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);//设置垂直居中对齐
			$objActSheet->getStyle($array[$i-1].'1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);//设置水平居中对齐
			if($dt_start == strtotime($begindate) || $dt_start == strtotime($enddate)){
				$objActSheet->getColumnDimension($array[$i-1])->setWidth(12);//设置每列日期宽度为12
				$objActSheet->setCellValue($array[$i-1].'1',date('Y-m-d',$dt_start)."\n".$week[$w]);
			}else{
				$day = date("d",$dt_start);
				if($day == 1){
					$objActSheet->getColumnDimension($array[$i-1])->setWidth(12);//设置每列日期宽度为12
					...
					$objActSheet->getColumnDimension($array[$i-1])->setWidth(3);//设置每列日期宽度为3
					$objActSheet->setCellValue($array[$i-1].'1',$day."\n".$week[$w]);
				}
			}
			$objFill = $objActSheet->getStyle($array[$i-1].'1')->getFill();
			$objFill->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			$objFill->getStartColor()->setARGB('FFEEEEEE');	//填充背景色
			//Set column borders 设置列边框 
		...
		for($i=0;$i<count($data);$i++){
			if($data[$i]['tasktype']!=$lasttype){//工作类别分组
				$cell = $array[$start].$d.":".$array[$end].$d;
				...
				$cell =  $array[$datecell].$d.":".$array[$dateendcell].$d;
				$objActSheet->mergeCells($cell);//合并单元格
			...
			while ($dt_start<=$dt_end){
				...
				if($dt_start>=strtotime($data[$i]['begindate']) && $dt_start<=strtotime($data[$i]['enddate'])){
					...
					if($data[$i]['finishper']==0)//根据完成率填充不同颜色
						...
						if($fillword==1){
							$objActSheet->setCellValue($array[$gantedate].$d,"[测试] 测试填入字体 刘凯");
							$fillword = 2;
						}
				}
				$dt_start = strtotime('+1 day',$dt_start);
