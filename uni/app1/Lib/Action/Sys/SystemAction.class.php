	...
	{		
		...
		if ($recordid)
		{
			if ($cancel=='true')
				...
				$sql="UPDATE `sys_share` SET `shareto`='$shareto' WHERE id='$recordid'";
			echo Data::sql($sql);
		}
		else
		{
			$orgid=System::orgid();
			$sql="INSERT INTO sys_share (`shareto`, `url`, `sharekey`,share_object_id,SYS_ORGID) VALUES ('$shareto','$url','$sharekey','$objectid ',$orgid)";
			$n=Data::sql($sql);
			if ($n==1)
